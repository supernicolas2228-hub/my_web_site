  ...
  return (
    <section id="faq" className="section-space px-3 sm:px-4">
      <div className="site-container max-w-3xl">
        <Reveal>
          <h2 className="font-heading text-3xl font-bold md:text-4xl">Часто задаваемые вопросы</h2>
        </Reveal>
        <Reveal delay={0.05}>
        ...
        <div className="mt-10 flex flex-col gap-4">
          {FAQ_ENTRIES.map((item, i) => (
            <Reveal key={item.id} delay={0.04 * i} y={16}>
              <details
                id={item.id}
                className="group glass-card overflow-hidden rounded-2xl border border-white/15 transition-[box-shadow,border-color] duration-300 open:border-indigo-400/40 open:shadow-lg open:shadow-indigo-500/15 dark:open:border-indigo-400/35"
              >
                <summary className="flex cursor-pointer list-none items-start justify-between gap-3 px-5 py-4 text-left text-base font-semibold leading-snug text-slate-900 marker:content-none dark:text-white md:px-6 md:py-5 md:text-lg [&::-webkit-details-marker]:hidden">
                  ...
                  <span
                    className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/25 bg-white/10 text-[0.65rem] leading-none text-indigo-600 transition duration-300 group-open:rotate-180 dark:border-white/15 dark:bg-white/5 dark:text-indigo-300"
                    aria-hidden
                  >
                    &#9660;
                  </span>
